...
3. **tier 에이전트 스폰** — `subagent_type: explorer-low` 1회 호출 → haiku/low 적용 확인. **단, 지금은 이 3번이 수행 불가하다.** 현재 `enforce-subagent-model.cjs`의 `MODEL_PINNED_TYPES`에 tier 에이전트 5종(explorer-low·executor-med·executor-high·reviewer-high·judge-max)이 등록돼 있지 않아, `model` 없이 tier를 호출하면 exit 2로 차단된다. 우회하려고 `model`을 넘기면 통과는 하지만 그 순간 호출 파라미터가 frontmatter의 `model`을 덮어써(`effort`·`tools`·`disallowedTools`는 유지) "pinned 조합이 그대로 적용됐는지"를 검증한다는 3번의 목적 자체가 성립하지 않는다. **이것은 재설치가 뭔가를 깨뜨린 것이 아니라 재설치 이전부터 있던 기존 제약이다** — 오퍼레이터가 이 3번에서 막혔다고 방금 덮어쓴 트리를 되돌리거나 훅을 직접 손대지 말 것. 해소 전까지는 1·2번만 수행하고 3번은 스킵한다. 추적: [../BACKLOG.md](../BACKLOG.md) **B-06**.
