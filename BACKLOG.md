...
**현재 상태 요약 (2026-08-21, PR #14 반영)**: 총 17건 중 15건 해결/해소(B-01·B-02·B-03·B-06·B-07·B-08·B-09·B-10·B-11·B-12·B-13·B-14·B-15·B-16·B-17). 남은 2건은 B-04(플러그인화, 트리거 미도달로 착수 보류), B-05(fable 트리거 5종 실전 검증 기록, 조치 불필요·관찰 기록)뿐이다. B-15·B-17은 "고쳐서" 닫힌 게 아니라 **롤백 서브시스템(`do_rollback`) 자체를 걷어내며 그 결함이 있던 표면이 사라져서** 닫혔다 — 복구 모델을 "백업에서 자동 복원"에서 "정본을 원하는 커밋으로 되돌려 재설치"로 바꿨다(`docs/REINSTALL.md` §5, `gotchas.md` 참고). B-16은 이 전환 전에 이미 단독 봉합됐었는데(매니페스트 설계 없이도 rm/복원 대칭화만으로 고칠 수 있는 결함이었다), 그 수정 코드도 이번에 롤백과 함께 사라졌다 — B-16 항목에 그 사실을 덧붙였다. 해결/해소 항목들은 삭제하지 않고 이력 보존용으로 남겨둔다.
...
**상태**: 미착수 · **우선순위**: 4 · **판정**: 트리거 도달 전까지 착수하지 않는다

skill + agents + hook을 하나의 Claude Code 플러그인 매니페스트로 묶어 여러 레포에 재사용하는 경로. 2026-08-17 이관 시점에는 미구현 — 로컬 `.claude/` 복사(경로 A)만 존재한다.

**착수 트리거 (명확한 숫자)**: 설치처가 **3곳을 초과**하면 착수한다. 현재 설치처는 **2곳**(정본 레포 + goone-rest) — 아직 트리거에 도달하지 않았다.

**착수 시 동반 필수 작업 목록 (한 PR로 묶어야 함, 현재 곳곳에 흩어진 항목을 한곳에 모음)**:
1. 훅 `enforce-subagent-model.cjs`의 `MODEL_PINNED_TYPES` 네임스페이스 갱신 — `subagent_type`이 `delegate-first:*` 형태로 바뀌면 비수식 일반명만 등록된 현행 목록에서 **차단된다**(실측 확인). 안 하면 B-06이 새 이름으로 재발.
2. B-11의 `TIER_EXPECTED_MODEL` map 키 갱신 — tier 이름이 네임스페이스화되면 기존 키와 불일치.
3. 린터(`scripts/lint-delegate-first.py`)의 `:` 포함 항목 면제 로직(`if ":" in name: continue`) 재검토 — 현재 네임스페이스 항목은 정방향 검사(pinned↔frontmatter model 정합성, Check A/B-07)에서 면제되어 FAIL이 조용히 사라진다(실측: 비수식명 FAIL 1/exit 1 → `delegate-first:*`로 바꾸면 FAIL 0/exit 0). 역방향 검사는 WARN으로 강등된 채 남는다(`--strict`에서만 exit 1, 기본 모드에선 커밋을 막지 못함).
4. 전역 훅·규칙 재전파(`~/.claude/hooks/`, `~/.claude/rules/`).
5. 설치처 전체(정본 + goone-rest 등) 재설치.
6. 위 1~5를 **한 PR/승인 대화에서 동기 갱신** — 한쪽만 고치면 desync가 새로 생긴다(B-08에서 동일 패턴 확인).

**보류 근거**: 트리거(설치처 3곳 초과) 미도달(현재 2곳). 게이트 2명이 독립적으로 보류에 동의했다 — 사유는 지금 플러그인화하면 방금 맞춘 3자 동기화(훅 pinned 목록·B-11 기대값 map·린터 네임스페이스 처리)를 **이득 없이 전면 재작업**하게 되기 때문이다.
...
**이 기록이 방어하는 대상**: 향후 fable 강제 트리거 5종을 축소·완화하자는 논의가 나오면, 그 논의는 먼저 이 실전 기록(8회 중 2회 REFUTED)을 반박해야 통과한다 — 트리거가 실제로 문제를 잡아낸 실증 사례이기 때문이다.
...
**할 일(완료, 위 "해소 내용" 참조)**: 파서를 문자열 연결까지 인식하도록 넓힐지, 아니면 "Set 항목은 리터럴 문자열만 허용"을 훅 파일 자체의 컨벤션으로 못박고 파서 확장은 보류할지 결정이 필요했다 — **후자로 결정**됐다(파서 확장 대신 "정적 리터럴이 아니면 FAIL로 표면화" 방식 채택, PR #9).

---

## B-10 — pre-commit을 `--strict`로 승격할지 결정

**상태**: 해결 (2026-08-19, PR #10) — WARN을 클래스 분리한 뒤 `--strict`로 승격했다. · **우선순위**: 5

**결정**: wholesale 승격(기존 `--strict`를 그대로 pre-commit에 연결) 대신, WARN을 `drift`(pinned↔agents 파일 불일치, model은 있는데 effort가 없음 등)와 `workflow`(위임 로그 `(리뷰 대기)` 미마감 행이 임계 초과)로 먼저 나누고, `--strict`가 **drift만** exit 1로 승격하도록 `scripts/lint-delegate-first.py`를 바꾼 뒤 `.githooks/pre-commit`의 린터 호출을 `--strict`로 올렸다. `Finding`에 `kind` 필드를 추가해 WARN 생성 시 클래스 지정을 강제(누락 시 즉시 예외)했고, 출력에는 `[WARN:drift]`/`[WARN:workflow]`로 사람이 구분할 수 있게 표시한다.

**wholesale 승격을 택하지 않은 이유**: `(리뷰 대기)` 미마감 초과는 결함이 아니라 정상 진행 중 작업 상태다. 아래에 남아 있던 "대가" 문단이 바로 이 문제였다 — WARN을 통째로 승격하면 위임이 진행 중이라는 이유만으로 무관한 커밋까지 막히므로, 이는 결함 게이트가 아니라 워크플로 상태에 대한 오탐 차단이 된다. 반면 pinned↔agents 드리프트류는 방치하면 훅의 판정 기준 자체가 실제 tier 정의와 어긋나는 진짜 결함이라 게이트할 가치가 있다.

**classification 기준(workflow 클래스는 2가지)**: `kind="workflow"`은 단일 사유가 아니라 "결함인가, 작업 상태인가"라는 하나의 판단 기준으로 묶인 두 케이스다 — ① 위임 로그 `(리뷰 대기)` 미마감 행이 임계 초과(정상 진행 중인 위임이 쌓이는 것) ② 위임 로그 데이터 행이 0건인 빈 로그(신규 설치 직후 부트스트랩 상태). 후자는 최초 drift로 잘못 분류돼 있었다 — 빈 로그는 구조적 결함이 아니라 "아직 위임을 한 번도 기록하지 않은 정상 상태"이므로, drift로 두면 갓 설치한 프로젝트가 `--strict`에서 첫 커밋부터 자기 게이트에 막혀 B-10이 애초에 피하려던 범주 오류("정상 작업 상태가 커밋을 막는 것")를 그대로 재현한다. 이 정정으로 `kind="workflow"`로 바로잡았다.

**검증**: `scripts/test-lint.sh`에 B-10 전용 이중 확인(기본/`--strict`) 4케이스(워크플로류 임계 초과, 워크플로류 빈 로그, 드리프트류 미pinned tier, 무변형 사본) + 비공허성 확인 2건(클래스 분리를 되돌린 사본에서 워크플로류 임계초과 케이스가, kind만 drift로 되돌린 사본에서 빈 로그 케이스가 각각 실제로 exit 1로 뒤집히는지 확인)을 추가했다 — 총 33/33 PASS. `bash .githooks/pre-commit`을 이 레포 현재 상태(WARN 0건)에서 실행해 통과(exit 0) 확인.
...
**할 일(완료, 위 "해소 내용" 참조)**: map 자료구조 설계 + 값 불일치 차단 로직 + break-glass 경로 + 스모크 테스트(`scripts/smoke-hook.sh`) 케이스 추가 — 전부 PR #9로 반영됨.
...
**후속 검토(완료)**: 당시엔 이 절차가 스크립트가 아니라 사람이 복붙하는 bash 블록이라 린트 대상이 아니었다 — 문서를 아무리 정확히 고쳐도 다음 실행자가 그대로 복붙하지 않으면 같은 사고가 재발할 위험이 있었다. REINSTALL 절차를 `scripts/reinstall.sh`로 스크립트화해 승계·검증·롤백을 코드로 강제할지는 후속 검토 사항이었으나, **PR #9로 스크립트화됨**(위 상태 줄 참조) — 더 이상 사람이 복붙하는 bash 블록에만 의존하지 않는다.
...
## B-15 — 백업 부분소실 시 silent partial restore (P4, skills·agents 양축)

**상태**: 해소 (2026-08-21, PR #14) — 롤백 제거로 표면 소멸. `do_rollback` 자체(및 이 항목이 가리키던 P4/P1 가드, pre-rollback 스냅샷)를 걷어냈다 — 고친 게 아니라 그 경로가 없어졌다. 이 항목이 우려하던 "부분 소실된 백업으로 live를 덮어쓴다"는 실패 모드는 애초에 "백업에서 자동 복원한다"는 계약이 있어야 성립하는데, 그 계약 자체를 없앴다(복구 = 정본을 원하는 커밋으로 되돌려 재설치, `docs/REINSTALL.md` §5). §1의 백업은 이제 가드·판정 없는 단순 `cp` 사본이라 완전성 검증이라는 개념 자체가 적용되지 않는다. 상세 배경은 `gotchas.md` "2026-08-21 — git이 이미 가진 이력을 복원하려고 기계를 쌓았다" 참고.

**상태 (이력, PR #14 이전)**: 미해결 — 보류(2026-08-21 codex critic 발견). · **우선순위**: 2

**증상**: `do_rollback`의 P4 가드(`scripts/reinstall.sh:222-266`)는 백업이 "완전한지"가 아니라 "완전히 빈 게 아닌지"만 확인한다 — skills 축은 `skills-delegate-first/SKILL.md` **1개 파일**만 있으면 통과하고(`:222-227`), agents 축은 tier 5종 중 **1개**만 있으면 통과한다(`:245-264`, `AGENTS_HAS_TIER_FILE` 루프가 `break`로 첫 매치에서 멈춘다). 백업이 생성 후 사후 훼손돼 예를 들어 `references/` 전체나 tier 파일 3종이 지워지고 SKILL.md/tier 1종만 남으면, 이 가드는 통과시키고 `do_rollback`은 rc=0으로 진행해 live의 온전한 스킬/에이전트를 그 훼손된 백업 내용으로 덮어쓴다 — **부분 소실이 부분 소실로 조용히 전파**된다.

**코드 위치**: `scripts/reinstall.sh:222-266`(P4/P1 가드, 존재 확인만 하고 완전성은 확인 안 함). 파괴 단계는 `:333-373`(pre-rollback 스냅샷, `PRE_ROLLBACK_DIR="$BACKUP_ROOT/pre-rollback-$PRE_ROLLBACK_STAMP"`)에서 롤백 직전 live 트리를 별도로 찍어두므로, 이 결함으로 잘못 덮어써진 뒤에도 **회수는 가능**하다 — 로그에 찍히는 `--rollback "$PRE_ROLLBACK_DIR" --dst "$DST"`로 되돌릴 수 있다. 즉 데이터가 영구 소실되지는 않지만, 사람이 알아채고 그 스냅샷 경로로 재복구를 실행해야 한다.

**왜 지금 안 고치는가**: 후보로 검토한 매니페스트 방식(백업 시점 파일 목록을 기록해 롤백 시 대조)이 "내용이 같은 자리에서 훼손"(예: SKILL.md는 그대로 있는데 내용만 잘림)까지는 못 잡는다 — 존재 여부가 아니라 **내용**까지 검증하려면 체크섬이 필요한데, 그 설계가 아직 없다. 자동 폴백(파일 일부 없으면 조용히 legacy/구버전 모드로 강등)도 검토했으나 이는 검사 자체를 우회하는 길이라 codex·Architect 리뷰 모두 "현재안으로는 승인 불가" 판정을 냈다. 파괴 경로(롤백)에 세 번째 상태 파일(매니페스트)을 늘리는 것도 이 브랜치의 방침("파괴 경로에 가드·분기를 늘리지 않는다")과 충돌해 이번 라운드에서는 보류한다.

**고칠 때 함께 해야 하는 것**: 일반 백업(§1)과 pre-rollback 스냅샷(§4, `:333-373`) **양쪽 다** 생성 시점에 매니페스트를 함께 만들어 path/type/checksum을 기록한다(symlink는 target까지 포함). 자동 legacy 폴백은 금지하고 — 매니페스트가 없는 구버전 백업을 롤백 소스로 쓰려면 명시적 legacy 모드 플래그가 필요하며, 비대화형(`--yes`)에서는 legacy 모드 자체를 거부한다. 매니페스트를 "이 백업은 완료된 백업이다"의 유일한 증거로 삼아, 매니페스트가 없는 백업은 기본적으로 거부한다(현재의 "파일 1개만 있으면 통과" 완화 없이). B-17도 같은 결함군(agents 축 복원 로직)이므로 **같은 PR에서 함께** 다룬다 — B-16(복원 범위 비대칭)은 매니페스트 설계 없이 먼저 봉합됐다(2026-08-21, PR #13).

## B-16 — 롤백이 커스텀 에이전트를 덮어쓴다 (codex 신규 발견)

**상태**: 해결 (2026-08-21, PR #13) — 복원 범위를 tier 5종(`TIER_AGENTS`)으로 좁혀 삭제(`rm -f`) 범위와 대칭을 맞췄다(새 가드 추가 아니라 기존 범위 초과 복원의 축소). 재현 시나리오는 아래 그대로 회귀 케이스 `caseB16_rollback_preserves_custom_agent_edit`로, 비공허성은 `caseB16_nonvacuous_broken_b16_reproduces_bug`(pre-fix `cp -R backup/agents/. dst/agents/` 라인을 되살린 사본에서 동일 시나리오가 실패함을 확인)로 잠갔다. · **우선순위**: 2

**추가 (2026-08-21, PR #14)**: 이 수정이 지켰던 코드(`do_rollback`의 에이전트 복원 분기)와 그 회귀 케이스(`caseB16_*`)는 롤백 자체를 걷어내며 함께 삭제됐다 — 여기 기록된 해결은 여전히 유효하지만, 지금은 "고쳐진 채로 남아 있는 코드"가 아니라 "그 코드가 더 이상 존재하지 않는다"는 뜻이다.

**증상**: 롤백 로그(`scripts/reinstall.sh:392`)는 "에이전트 복원 (F-3: overlay가 아니라 교체 복원)"이라 말하고, 바로 아래 F-3 주석(`:394-401`)도 "재설치가 실제로 건드리는 대상인 tier 5종만 먼저 지우고, 백업 내용으로 다시 채운다"고 tier 5종 한정을 명시한다. 그런데 실제 복원 라인(`:405`)은 `cp -R "$ROLLBACK_DIR/agents/." "$DST/.claude/agents/"` — **백업의 agents 디렉터리 전체**를 dst로 복사한다. 재설치가 건드리지도 않는 사용자 커스텀 에이전트 파일이 dst에 있으면, 백업 시점 이후 그 파일을 편집했더라도 롤백이 **백업 시점 내용으로 조용히 되돌린다**.

**실측 재현**(2026-08-21, `mktemp -d` 샌드박스, `REINSTALL_HOME` 격리): 정상 설치 → 백업 생성 → 백업 이후 `my-custom-agent.md`를 "v1"에서 "v2"로 편집(재설치와 무관한 사용자 작업) → `--rollback` 실행 → 롤백 후 파일 내용이 "v2"가 아니라 백업 시점의 "v1"로 되돌아감을 확인. 계획-실행 불일치(F-2와 같은 계열: 로그는 좁은 범위를 말하는데 실행은 넓은 범위를 건드림)이자 범위 초과 복원이다.

**코드 위치**: `scripts/reinstall.sh:391-405`(에이전트 복원 분기 전체, 특히 `:405`의 무조건 `cp -R ... /.`).

**고칠 때**: 복원 범위를 tier 5종(`TIER_AGENTS`)으로 명시적으로 한정한다 — `cp -R backup/agents/. dst/agents/` 대신 tier 5개 파일 각각에 대해서만 "백업에 있으면 복사, 없으면 지운 채로 둔다"를 개별 실행한다(현재도 삭제는 이미 그렇게 tier 한정으로 하고 있으니, 복사도 대칭으로 한정하면 된다). 회귀 테스트로 커스텀 에이전트 checksum이 롤백 전후로 불변임을 잠근다(B-15와 같은 PR에서).

## B-17 — custom-only agents 디렉터리를 손상 백업으로 오인 (codex 신규 발견)

**상태**: 해소 (2026-08-21, PR #14) — 롤백 제거로 표면 소멸. 이 항목이 가리키던 P1 가드(`scripts/reinstall.sh:245-264`)는 `do_rollback` 안에 있었고, `do_rollback` 자체를 걷어내며 함께 사라졌다 — 고친 게 아니라 그 경로가 없어졌다. "custom-only agents/를 손상 백업으로 오인해 거부한다"는 실패 모드는 "백업 완전성을 판정해 자동 복원 여부를 가른다"는 계약이 있어야 성립하는데, 그 계약 자체를 없앴다(복구 = 정본을 원하는 커밋으로 되돌려 재설치, `docs/REINSTALL.md` §5). 상세 배경은 `gotchas.md` "2026-08-21 — git이 이미 가진 이력을 복원하려고 기계를 쌓았다" 참고.

**상태 (이력, PR #14 이전)**: 미해결 — 보류(2026-08-21 codex critic 발견, B-15와 같은 PR에서 처리). · **우선순위**: 2

**증상**: B-15의 P1 가드(`scripts/reinstall.sh:245-264`)는 백업의 `agents/` 디렉터리가 존재하는데 tier 5종 파일이 **하나도 없으면** "내용 소실"로 판정해 롤백 전체를 거부한다(`:259-263`). 그런데 "이 프로젝트의 tier 5종을 한 번도 설치한 적이 없고, agents/ 안에 사용자 커스텀 에이전트만 있는" 상태는 정당한 정상 상태다(예: 이 프로젝트를 설치하기 **전부터** 다른 에이전트를 쓰던 사용자가 백업을 찍었는데, 그 시점엔 아직 tier 5종이 없었던 경우) — 이 경우 `AGENTS_FRESH_INSTALL_MARKER`도 없다(마커는 §1이 "agents/ 디렉터리 자체가 없었을 때"만 찍는데, 여기선 디렉터리는 있고 내용만 tier 미포함이므로 마커 조건에 해당 안 함). 결과적으로 가드는 이 정당한 백업을 손상으로 오인해 롤백 자체를 **거부**한다(false-refusal) — F-1/P1이 막으려던 "내용 소실을 신규 설치로 오인"과는 반대 방향의 오탐이다.

**실측 재현**(2026-08-21, `mktemp -d` 샌드박스): `agents/` 디렉터리가 있고 커스텀 에이전트 파일 1개만 든 백업(tier 파일 0개, 마커 없음)으로 `--rollback` 실행 → `백업 불완전: .../agents 디렉터리는 있지만 tier 에이전트 파일이 하나도 없다` 에러로 rc=1 거부됨을 확인. 정당한 롤백을 막는 결함이다.

**코드 위치**: `scripts/reinstall.sh:245-264`(P1 가드).

B-16 수정으로 복원 범위가 좁아졌지만, custom-only 백업을 손상으로 오인해 **거부**하는 문제는 그대로다(B-16은 "복원 시 무엇을 덮어쓰는지"를 고쳤을 뿐, 이 가드가 "tier 파일이 하나도 없으면 무조건 거부"하는 판정 로직 자체는 건드리지 않았다).

**고칠 때**: B-15의 매니페스트 방식이 들어오면 "완료된 백업"을 매니페스트 존재로 판정할 수 있으므로 이 구분(진짜 소실 vs. 원래 tier가 없었던 정상 상태)이 자연히 풀린다 — 별도 마커를 또 늘리기보다 B-15 설계에 이 케이스를 포함시켜 B-15와 **같은 PR**에서 함께 다룬다.
