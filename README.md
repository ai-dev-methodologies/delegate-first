...
2. **훅을 별도로 등록한다** — A) 절차든 `scripts/reinstall.sh`든 이 단계를 대신해주지 않는다.
   - 먼저 훅 **파일**을 배치한다. 1번을 `scripts/reinstall.sh`로 처리했다면 아래 A) 블록(수동 `cp`)을 실행하지 않았을 것이므로, 이 `cp`를 생략하면 훅 **파일 자체가 존재하지 않는** 상태로 다음 줄의 `settings.json` 등록만 하게 된다(등록할 파일이 없다). 프로젝트 로컬에 둘지 사용자 홈(전역)에 둘지 고른 뒤 하나만 실행한다:
     ```bash
     # 프로젝트 로컬:
     ...
     # cp "$SRC/.claude/hooks/enforce-subagent-model.cjs" "$HOME/.claude/hooks/"
     ```
   - 그다음 `settings.json`의 `PreToolUse`에 등록한다. `.claude/settings.json.example`을 복사해 등록하는 절차는 아래 A) 안의 "훅은 파일을 두는 것만으로는 작동하지 않는다" 문단을 따른다.
